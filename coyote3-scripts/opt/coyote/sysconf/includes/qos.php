...
define("QOS_PRIO_LOW", 30);
	...
	function StartQoS($Config) {
		...
		do_exec("tc qdisc add dev $pi root handle 1: htb default $dp");
		do_exec("tc class add dev $pi parent 1: classid 1:1 htb rate $up_max burst 15k");
		do_exec("tc class add dev $pi parent 1:1 classid 1:10 htb rate $up_high ceil $up_max burst 15k");
		do_exec("tc class add dev $pi parent 1:1 classid 1:20 htb rate $up_norm ceil $up_ceil burst 15k");
		do_exec("tc class add dev $pi parent 1:1 classid 1:30 htb rate $up_low ceil $up_ceil burst 5k");
		do_exec("tc qdisc add dev $pi parent 1:10 handle 10: sfq perturb 10");
		do_exec("tc qdisc add dev $pi parent 1:20 handle 20: sfq perturb 10");
		do_exec("tc qdisc add dev $pi parent 1:30 handle 30: sfq perturb 10");
		do_exec("tc filter add dev $pi parent 1:0 protocol ip prio 1 handle 10 fw flowid 1:10");
		do_exec("tc filter add dev $pi parent 1:0 protocol ip prio 1 handle 20 fw flowid 1:20");
		do_exec("tc filter add dev $pi parent 1:0 protocol ip prio 1 handle 30 fw flowid 1:30");

		// Set up the internal interface
		do_exec("tc qdisc add dev $li root handle 1: htb default $dp");
		do_exec("tc class add dev $li parent 1: classid 1:1 htb rate $dl_max burst 15k");
		do_exec("tc class add dev $li parent 1:1 classid 1:10 htb rate $dl_high ceil $dl_max burst 15k");
		do_exec("tc class add dev $li parent 1:1 classid 1:20 htb rate $dl_norm ceil $dl_ceil burst 15k");
		do_exec("tc class add dev $li parent 1:1 classid 1:30 htb rate $dl_low ceil $dl_ceil burst 5k");
		do_exec("tc qdisc add dev $li parent 1:10 handle 10: sfq perturb 10");
		do_exec("tc qdisc add dev $li parent 1:20 handle 20: sfq perturb 10");
		do_exec("tc qdisc add dev $li parent 1:30 handle 30: sfq perturb 10");
		do_exec("tc filter add dev $li parent 1:0 protocol ip prio 1 handle 5 fw flowid 1:1");
		do_exec("tc filter add dev $li parent 1:0 protocol ip prio 1 handle 10 fw flowid 1:10");
		do_exec("tc filter add dev $li parent 1:0 protocol ip prio 1 handle 20 fw flowid 1:20");
		do_exec("tc filter add dev $li parent 1:0 protocol ip prio 1 handle 30 fw flowid 1:30");
